...
exports.uploadToPublic = function(req, res) {
	...
	*/
	...
	thumbnail.forEach(function(v,i){
		// 获得文件的临时路径
		var tmp_path = v.path;
		// 指定文件上传后的目录 - 示例为"images"目录。
		var file_name = v.name,
			ext;
		if(file_name && (ext = path.extname(file_name))){
			var imgPath = '/images/' + md5(file_name+new Date().getTime()) + '.png';
			...
			fs.rename(tmp_path, target_path, function(err) {
				...
				fs.unlink(tmp_path, function() {
					if(err) throw err;
					res.send('File uploaded to: ' + target_path + ' - ' + v.size + ' bytes'+'<br/><img src="'+imgPath+'"/>');
				});
			});
		}
	});
